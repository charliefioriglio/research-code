...
if __name__ == '__main__':
    ...
    for E in E_eV:
        ...
        A_par = compute_average_amplitudes(X, Y, Z, DO_coeffs_b3g, DO_coeffs_b3g, ang_grid, 0, 0, E_au)
        deg_A_par = compute_average_amplitudes(X, Y, Z, DO_coeffs_b2g, DO_coeffs_b2g, ang_grid, 0, 0, E_au)

        A_perp1 = compute_average_amplitudes(X, Y, Z, DO_coeffs_b3g, DO_coeffs_b3g, ang_grid, 0, 0, E_au)
        A_perp2 = compute_average_amplitudes(X, Y, Z, DO_coeffs_b3g, DO_coeffs_b3g, ang_grid, 0, 0, E_au)
        deg_A_perp1 = compute_average_amplitudes(X, Y, Z, DO_coeffs_b2g, DO_coeffs_b2g, ang_grid, 0, 0, E_au)
        deg_A_perp2 = compute_average_amplitudes(X, Y, Z, DO_coeffs_b2g, DO_coeffs_b2g, ang_grid, 0, 0, E_au)

        # intensities
        sigma_par = np.real(np.mean(A_par) + np.mean(deg_A_par))
        sigma_perp = 0.5 * np.real(np.mean(A_perp1) + np.mean(A_perp2) + np.mean(deg_A_perp1) + np.mean(deg_A_perp2))
