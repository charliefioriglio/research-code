...
# ---------------------------------------------
def compute_average_amplitudes(X, Y, Z, DO_coeffs_L, DO_coeffs_R, ang_grid, k_vec, pol_lab):
    N = ang_grid.shape[0]
    amps = np.zeros(N, dtype=complex)
    
    # Use fixed lab-frame polarization and k vector
    mu = pol_lab[0]*X + pol_lab[1]*Y + pol_lab[2]*Z
    psi_el = plane_wave(k_vec, X, Y, Z)
...
if __name__ == '__main__':
    ...
    for E in E_eV:
        ...
        # compute amplitudes
        A_par = compute_average_amplitudes(X, Y, Z, DO_coeffs_sig, DO_coeffs_sig, ang_grid, k_z_lab_scaled, pol_lab_z)
        A_perp1 = compute_average_amplitudes(X, Y, Z, DO_coeffs_sig, DO_coeffs_sig, ang_grid, k_z_lab_scaled, pol_lab_x)
        A_perp2 = compute_average_amplitudes(X, Y, Z, DO_coeffs_sig, DO_coeffs_sig, ang_grid, k_z_lab_scaled, pol_lab_y)
